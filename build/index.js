            ...
            try {
                ...
                switch (name) {
                    ...
                    case "get_earnings_ticker":
                        ...
                        result = await this.api.getEarningsTicker(args.ticker);
                        break;
                    // ETFs
                    case "get_etf_exposure":
                        ...
                        result = await this.api.getETFExposure(args.ticker);
                        break;
                    case "get_etf_holdings":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getETFHoldings(args.ticker);
                        break;
                    case "get_etf_in_outflow":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getETFInOutflow(args.ticker);
                        break;
                    case "get_etf_info":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getETFInfo(args.ticker);
                        break;
                    case "get_etf_weights":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getETFWeights(args.ticker);
                        break;
                    // Market
                    ...
                    case "get_stock_info":
                        ...
                        result = await this.api.getStockInfo(args.ticker);
                        break;
                    case "get_stock_flow_alerts":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockFlowAlerts(args.ticker);
                        break;
                    case "get_stock_flow_recent":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockFlowRecent(args.ticker);
                        break;
                    case "get_stock_option_chains":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockOptionChains(args.ticker);
                        break;
                    case "get_stock_greek_exposure":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockGreekExposure(args.ticker);
                        break;
                    case "get_stock_max_pain":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockMaxPain(args.ticker);
                        break;
                    case "get_stock_iv_rank":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockIVRank(args.ticker);
                        break;
                    case "get_stock_volatility_stats":
                        if (!args?.ticker) {
                            throw new McpError(ErrorCode.InvalidParams, "ticker parameter is required");
                        }
                        result = await this.api.getStockVolatilityStats(args.ticker);
                        break;
                    default:
